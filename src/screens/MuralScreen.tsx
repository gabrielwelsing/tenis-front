  ...
  return (
    <div style={rg.card}>
      <button onClick={() => setAberto(v => !v)} style={rg.header}>
        ...
        <span style={rg.furosText}>Seus furos: <strong style={{ color: furos >= 3 ? '#c95441' : '#3f8f5b' }}>{furos}</strong> {aberto ? '▲' : '▼'}</span>
      </button>
      {aberto && (
        <div style={rg.body}>
...
export default function MuralScreen({ onBack, emailUsuario, userId, username, telefone, localidade }: Props) {
  ...
  const loadJogos = useCallback(() => {
    ...
    getJogos(cidade).then(data => setJogos(data as Jogo[])).catch(() => setJogos([])).finally(() => setLoadingJogos(false));
  }, [cidade]);

  useEffect(() => { loadJogos(); }, [loadJogos]);

  const handleConfirmCity = useCallback((c: string) => { localStorage.setItem(LS_CIDADE, c); setCidade(c); setShowCity(false); }, []);
  ...
  const handleInteressado = useCallback(async (jogoId: string) => {
    try {
      await fetch(`${API_BASE}/jogos/${jogoId}/interessado`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email_usuario: emailUsuario, nome_usuario: username }) });
  ...
  const handleConfirmarSala = useCallback(async (jogoId: string, confirmado_com: string) => {
    await fetch(`${API_BASE}/jogos/${jogoId}/confirmar`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email_publicador: emailUsuario, confirmado_com }) });
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.header}>
        ...
        <button type="button" onClick={() => setShowPublicar(true)} style={{ ...s.publishTopBtn, opacity: banStatus.banido ? 0.45 : 1, cursor: banStatus.banido ? 'not-allowed' : 'pointer' }} disabled={banStatus.banido}>
          + Publicar
        </button>
      </div>
      ...
      <div style={s.scrollBody}>
        <div style={s.inner}>
          <div style={s.cityRow}>
            <button onClick={() => setShowCity(true)} style={s.cidadeBtn}>📍 {cidade || 'Selecionar cidade'}</button>
          ...
          <section style={s.section}>
            <div style={s.filterRow}>
              {[
                ...
                <button key={item.key} type="button"
                  ...
                  onClick={() => { const next = item.key as FiltroMural; setFiltro(next); if (next !== 'calendario') setSelectedDate(null); }}>
                  {item.label}
                </button>
              ))}
            </div>

            {filtro === 'calendario' && <MiniCalendar jogos={jogosAtivos} selectedDate={selectedDate} onSelectDate={setSelectedDate} />}

            <div style={s.listHeader}>
              <div>
                <h2 style={s.sectionTitle}>Jogos disponíveis</h2>
                <p style={s.sectionSub}>{jogosExibidos.length} publicaç{jogosExibidos.length === 1 ? 'ão' : 'ões'}{filtro === 'calendario' && selectedDate ? ` em ${fmtData(selectedDate)}` : ` em ${cidade || '…'}`}</p>
              </div>
            </div>
            ...
            {loadingJogos ? (
              ...
              <div style={s.emptyFeed}>
                ...
                <p style={s.emptyText}>{filtro === 'calendario' && selectedDate ? `Nenhum parceiro em ${fmtData(selectedDate)}.` : filtro === 'meus' ? 'Você ainda não publicou nenhum treino.' : `Nenhum parceiro disponível em ${cidade || 'sua cidade'} ainda.`}</p>
                <p style={s.emptyHint}>Clique em "Publicar" para criar uma disponibilidade.</p>
              </div>
            ) : (
              <div style={s.feed}>
                {jogosExibidos.map(jogo => (
                  <JogoCard key={jogo.id} jogo={jogo} furosReportados={furosMap[jogo.whatsapp] || 0}
                    ...
                    emailUsuario={emailUsuario} userId={userId} />
                ))}
              </div>
            )}
          </section>
...
function PublishModal({
  ...
  dataInicio, setDataInicio, dataFim, setDataFim, horarioInicio, setHorarioInicio,
  horarioFim, setHorarioFim, local, setLocal, localOutro, setLocalOutro,
  whatsapp, setWhatsapp, erro, sucesso, onPublicar, onClose,
}: {
  cidade: string; hoje: string; banStatus: { banido: boolean; mensagem: string; permanente: boolean };
  classe: string; setClasse: (v: string) => void; janelaData: boolean; setJanelaData: (v: boolean) => void;
  dataInicio: string; setDataInicio: (v: string) => void; dataFim: string; setDataFim: (v: string) => void;
  horarioInicio: string; setHorarioInicio: (v: string) => void; horarioFim: string; setHorarioFim: (v: string) => void;
  local: string; setLocal: (v: string) => void; localOutro: string; setLocalOutro: (v: string) => void;
  whatsapp: string; setWhatsapp: (v: string) => void; erro: string; sucesso: boolean;
          ...
          <>
            ...
            <FieldGroup label="Disponibilidade de dias">
              <div style={pm.modeToggle}>
                <button type="button" style={{ ...pm.modeBtn, ...(janelaData ? {} : pm.modeBtnActive) }} onClick={() => setJanelaData(false)}>Dia único</button>
                <button type="button" style={{ ...pm.modeBtn, ...(janelaData ? pm.modeBtnActive : {}) }} onClick={() => setJanelaData(true)}>Janela de dias</button>
              </div>
            </FieldGroup>
            <div style={pm.row}>
              <div style={pm.col}>
            ...
            <FieldGroup label="Janela de horários">
              <div style={pm.timeStack}>
                <div style={pm.timeRow}><span style={pm.timeLabel}>Das</span><input type="time" value={horarioInicio} onChange={e => setHorarioInicio(e.target.value)} style={pm.timeInput} /></div>
                <div style={pm.timeRow}><span style={pm.timeLabel}>Às</span><input type="time" value={horarioFim} onChange={e => setHorarioFim(e.target.value)} style={pm.timeInput} /></div>
              </div>
            </FieldGroup>
            <FieldGroup label="Local">
              ...
              {local === 'Outro' && (
                <input style={{ ...pm.input, marginTop: 8 }} placeholder="Descreva o local..." type="text" value={localOutro} onChange={e => setLocalOutro(e.target.value)} autoCapitalize="words" />
              )}
            </FieldGroup>
            <FieldGroup label="Seu WhatsApp">
              <input type="tel" inputMode="numeric" placeholder="(33) 99999-0000" value={whatsapp} onChange={e => setWhatsapp(maskPhone(e.target.value))} style={pm.input} />
            </FieldGroup>
            {erro    && <p style={pm.erro}>{erro}</p>}
            {sucesso && <p style={pm.ok}>✅ Publicado! Aguardando parceiro…</p>}
            <button type="button" onClick={onPublicar} style={pm.publishBtn}>Publicar disponibilidade</button>
          </>
        )}
      </div>
    </div>
...
function JogoCard({ jogo, furosReportados, onReportarFuro, onInteressado, onConfirmarSala, emailUsuario, userId }: {
  jogo: Jogo; furosReportados: number; onReportarFuro: () => void; onInteressado: () => void;
  onConfirmarSala: (email: string) => void; emailUsuario: string; userId: number;
}) {
  ...
  const handleConfirmar = async () => {
    ...
    setConfirmando(true); onConfirmarSala(selectedEmail); setConfirmando(false); setShowInteress(false);
  };

  const jaJogou = isExpired(jogo);

  // Data para o box estilo referência
  const dataOnly = jogo.dataInicio.slice(0, 10);
  const [, , dia] = dataOnly.split('-');
  const dtObj     = new Date(dataOnly + 'T12:00:00');
  const mesAbrev  = MESES[dtObj.getMonth()].toUpperCase();
  const isHoje    = dataOnly === hojeStr();
  ...
  return (
    <>
      ...
      <div style={sc.card}>
        {/* Header: avatar + nome + tempo + badges */}
        <div style={sc.topRow}>
          <div style={sc.author}>
            {jogo.fotoPublicador
            ...
            <div style={sc.authorInfo}>
              ...
              <div style={sc.authorSub}>
                <span style={{ color: cor, fontSize: 10 }}>●</span> {jogo.classe}
              </div>
            </div>
          </div>
        ...
        {/* Info box: data | horário+local | interessados */}
        <div style={sc.infoBox}>
          {/* Coluna data */}
          <div style={sc.infoCell}>
            {isHoje && <span style={sc.hojeLabel}>HOJE</span>}
            <span style={sc.diaNum}>{dia}</span>
            <span style={sc.mesLabel}>{mesAbrev}</span>
            {jogo.dataFim && jogo.dataFim !== jogo.dataInicio && (
              <span style={sc.dataFimLabel}>até {fmtData(jogo.dataFim)}</span>
            )}
          </div>

          <div style={sc.divider} />

          {/* Coluna horário + local */}
          <div style={{ ...sc.infoCell, alignItems: 'flex-start', gap: 6 }}>
            <div style={sc.horarioRow}>
              <span style={sc.infoIcon}>🕐</span>
              <span style={sc.horarioText}>{jogo.horarioInicio.replace(':', 'h')} – {jogo.horarioFim.replace(':', 'h')}</span>
            </div>
            <div style={sc.horarioRow}>
              <span style={sc.infoIcon}>📍</span>
              <span style={sc.localText}>{jogo.local}</span>
            </div>
          </div>
          ...
          {/* Coluna interessados */}
          <div style={sc.infoCell}>
            <span style={sc.interessNum}>{jogo.interessados ?? 0}</span>
            <span style={sc.interessLabel}>interessado{(jogo.interessados ?? 0) === 1 ? '' : 's'}</span>
          </div>
        </div>

        {furosReportados >= 3 && <span style={sc.furoBadge}>⚠️ {furosReportados} furos reportados</span>}
        ...
        {!isConfirmada && (
          <div style={sc.btnRow}>
            ...
            <a href={buildGCalUrl(jogo)} target="_blank" rel="noopener noreferrer" style={sc.calBtn}>📅</a>
          </div>
        )}
                ...
                {loadingInteress ? (
                  ...
                  <>
                    <select style={sc.interessSelect} value={selectedEmail} onChange={e => setSelectedEmail(e.target.value)}>
                      ...
                      {interessados.map(i => <option key={i.email_usuario} value={i.email_usuario}>{i.nome_usuario}</option>)}
                    </select>
                    <button style={{ ...sc.confirmarBtn, opacity: !selectedEmail || confirmando ? 0.5 : 1 }} disabled={!selectedEmail || confirmando} onClick={handleConfirmar}>
                      Confirmar partida
                    </button>
                  </>
                )}
        ...
        {isConfirmada && isOwner && (
          <div style={sc.confirmadaInfo}>✓ Confirmado com <strong>{jogo.confirmado_com?.split('@')[0]}</strong></div>
        )}
...
function ResultadoModal({ jogo, emailUsuario, userId, onClose }: { jogo: Jogo; emailUsuario: string; userId: number; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/ranking/ligas`, { headers: { Authorization: `Bearer ${token}` } })
      .then(r => r.json()).then(d => { const arr: Liga[] = d.data ?? []; setLigas(arr); if (arr.length > 0) { setLigaId(arr[0].id); setTempId(arr[0].temporada_ativa_id ?? ''); } })
  ...
  if (ok) return (<div style={rm.overlay} onClick={onClose}><div style={rm.sheet}><div style={{ fontSize: 48 }}>🏆</div><p style={{ color: '#3f8f5b', fontWeight: 900, fontSize: 16, margin: 0 }}>Resultado registrado!</p><p style={{ color: '#8d7b70', fontSize: 13, margin: 0 }}>Aguardando confirmação do adversário.</p></div></div>);
  ...
  return (
    <div style={rm.overlay} onClick={onClose}>
      <div style={rm.sheet} onClick={e => e.stopPropagation()}>
        ...
        {!wo && (<><select style={rm.sel} value={tipo} onChange={e => setTipo(e.target.value)}><option value="melhor_de_3">Melhor de 3 sets</option><option value="2sets_supertiebreak">2 Sets + Super Tiebreak</option><option value="pro_set">Pró-set</option></select><p style={rm.hint}>Placar — Eu × Adversário</p>{Array.from({ length: show3 ? 3 : nSets }).map((_, i) => (<div key={i} style={rm.setRow}><span style={rm.setLabel}>Set {i+1}</span><input style={rm.setInp} type="number" min={0} max={99} placeholder="Eu" value={sets[i].a} onChange={e => setSets(prev => { const n = [...prev]; n[i] = { ...n[i], a: e.target.value }; return n; })} /><span style={{ color: '#a29186' }}>×</span><input style={rm.setInp} type="number" min={0} max={99} placeholder="Adv" value={sets[i].b} onChange={e => setSets(prev => { const n = [...prev]; n[i] = { ...n[i], b: e.target.value }; return n; })} /></div>))}</>)}
...
function FieldGroup({ label, children }: { label: string; children: React.ReactNode }) {
  return (<div style={pm.fieldGroup}><span style={pm.label}>{label}</span>{children}</div>);
}
    ...
    <div style={cal.wrapper}>
      ...
      <div style={cal.grid}>
        ...
        {cells.map((d, i) => {
          ...
          return (<button key={dateStr} onClick={() => clickable && onSelectDate(isSel ? null : dateStr)} style={{ ...cal.dayBtn, ...(isPast ? cal.dayPast : {}), ...(isToday && !isSel ? cal.dayToday : {}), ...(clickable && !isSel ? cal.dayHas : {}), ...(isSel ? cal.daySel : {}), cursor: clickable ? 'pointer' : 'default' }}>{d}{clickable && <span style={{ ...cal.dot, background: isSel ? '#fff' : '#c66b4d' }} />}</button>);
        })}
      </div>
...
const pm: Record<string, React.CSSProperties> = {
  ...
  timeInput: { flex: 1, minWidth: 0, padding: '13px 14px', borderRadius: 14, background: '#fff', border: '1px solid #eadfd6', color: '#332a25', fontSize: 15, fontWeight: 650, boxSizing: 'border-box', colorScheme: 'light' },
...
const s: Record<string, React.CSSProperties> = {
  ...
  publishTopBtn: { padding: '11px 14px', borderRadius: 14, border: 'none', background: 'linear-gradient(135deg, #c66b4d, #934836)', color: '#fff', fontSize: 13, fontWeight: 900, boxShadow: '0 10px 20px rgba(147,72,54,0.18)', whiteSpace: 'nowrap' },
...
const sc: Record<string, React.CSSProperties> = {
  card: { background: '#fff', border: '1px solid rgba(130,82,62,0.08)', borderRadius: 24, padding: 14, display: 'flex', flexDirection: 'column', gap: 12, boxShadow: '0 10px 30px rgba(117,76,56,0.08)' },

  // Header do card
  topRow: { display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10 },
  author: { display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 },
  avatar: { width: 44, height: 44, borderRadius: '50%', objectFit: 'cover', border: '2px solid #fff', boxShadow: '0 8px 18px rgba(90,54,39,0.12)' },
  avatarFallback: { width: 44, height: 44, borderRadius: '50%', background: 'linear-gradient(135deg, #c6714e, #8f4635)', color: '#fff', fontSize: 17, fontWeight: 950, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid #fff', boxShadow: '0 8px 18px rgba(90,54,39,0.12)' },
  authorInfo: { minWidth: 0, display: 'flex', flexDirection: 'column', gap: 3 },
  authorName: { color: '#2d2521', fontSize: 15, fontWeight: 950, lineHeight: 1.15, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' },
  authorSub: { color: '#8f7769', fontSize: 12, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 4 },
  topRight: { display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4, flexShrink: 0 },
  postTime: { color: '#a29186', fontSize: 11, fontWeight: 750 },
  ownerBadge: { fontSize: 10, fontWeight: 950, padding: '4px 8px', borderRadius: 999, background: '#fff0e8', color: '#a54f3d' },
  confirmadaBadge: { fontSize: 10, fontWeight: 950, padding: '4px 8px', borderRadius: 999, background: '#edf8ef', color: '#3f8f5b' },

  // Info box (data | horário | interessados)
  infoBox: { display: 'flex', alignItems: 'stretch', background: '#fdf5ef', borderRadius: 16, overflow: 'hidden', border: '1px solid rgba(198,107,77,0.1)' },
  infoCell: { flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 2, padding: '12px 8px' },
  divider: { width: 1, background: 'rgba(198,107,77,0.12)', flexShrink: 0 },
  hojeLabel: { fontSize: 9, fontWeight: 950, color: '#c66b4d', letterSpacing: 0.8, textTransform: 'uppercase' as const },
  diaNum: { fontSize: 26, fontWeight: 950, color: '#2d2521', lineHeight: 1 },
  mesLabel: { fontSize: 10, fontWeight: 850, color: '#8f7769', letterSpacing: 0.5 },
  dataFimLabel: { fontSize: 9, color: '#a29186', fontWeight: 700, textAlign: 'center' as const },
  horarioRow: { display: 'flex', alignItems: 'center', gap: 5 },
  horarioText: { fontSize: 13, fontWeight: 900, color: '#2d2521' },
  localText: { fontSize: 11, fontWeight: 750, color: '#8f7769', lineHeight: 1.3 },
  infoIcon: { fontSize: 13, flexShrink: 0 },
  interessNum: { fontSize: 22, fontWeight: 950, color: '#c66b4d', lineHeight: 1 },
  interessLabel: { fontSize: 10, fontWeight: 750, color: '#8f7769', textAlign: 'center' as const },

  furoBadge: { alignSelf: 'flex-start', fontSize: 11, fontWeight: 850, padding: '6px 10px', borderRadius: 999, background: '#fff0ec', color: '#c95441' },

  // Botões
  btnRow: { display: 'grid', gridTemplateColumns: '1fr 44px', gap: 8 },
  waBtn: { display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '12px', borderRadius: 14, background: 'linear-gradient(135deg, #c66b4d, #934836)', color: '#fff', fontSize: 13, fontWeight: 950, textDecoration: 'none', boxShadow: '0 8px 18px rgba(147,72,54,0.2)' },
  calBtn: { display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 14, background: '#f5ebe4', color: '#8f4a39', fontSize: 18, textDecoration: 'none' },
  ...
  interessBtn: { width: '100%', padding: '10px 12px', borderRadius: 13, background: '#fff', border: '1px solid rgba(130,82,62,0.1)', color: '#a54f3d', fontSize: 13, fontWeight: 900, cursor: 'pointer', textAlign: 'left' },
  interessBox: { display: 'flex', flexDirection: 'column', gap: 9 },
  mutedText: { color: '#9b8a7f', fontSize: 13, margin: 0, fontWeight: 700 },
  ...
  confirmarBtn: { width: '100%', padding: '12px', borderRadius: 14, background: 'linear-gradient(135deg, #c66b4d, #934836)', border: 'none', color: '#fff', fontSize: 13, fontWeight: 950, cursor: 'pointer' },
  confirmadaInfo: { fontSize: 13, color: '#3f8f5b', fontWeight: 800, textAlign: 'center', padding: '4px 0' },

  reportRow: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 },
  reportBtn: { background: 'none', border: 'none', padding: '4px 2px', color: '#b09a8e', fontSize: 11, cursor: 'pointer', textDecoration: 'underline', textUnderlineOffset: 2 },
  reportadoTxt: { fontSize: 11, color: '#3f8f5b', fontWeight: 800 },
  rankBtn: { background: '#fff4e8', border: '1px solid rgba(198,107,77,0.22)', color: '#a54f3d', borderRadius: 12, padding: '8px 10px', fontSize: 12, fontWeight: 900, cursor: 'pointer' },

  // Itens de info (mantidos para compatibilidade)
  infoItem: { display: 'flex', alignItems: 'center', gap: 7, minWidth: 0 },
  infoText: { color: '#55463f', fontSize: 12.5, fontWeight: 800, lineHeight: 1.2, minWidth: 0 },
};
